...
static sylar::ConfigVar<uint64_t>::ptr g_http_request_max_body_size = 
	sylar::Config::Lookup("http.reqeust.max_body_size"
...
struct _RequestSizeIniter {
	_RequestSizeIniter() {
		...
		s_http_request_max_body_size = g_http_request_buffer_size->getValue();
...
void on_request_http_field(void *data, const char *field, size_t flen, const char *value, size_t vlen) {
	HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
	if (flen == 0) {
		SYLAR_LOG_WARN(g_logger) << "invalid http reqeust field length = 0";
...
//1: 成功 
//-1: 有错误 
//>0: 已经处理的字节数, 且data有效数据为len - v 
size_t HttpRequestParser::execute(char* data, size_t len) {
	size_t offset = http_parser_execute(&m_parser, data, len, 0);
		//设定的4K, 解完4k后如果还剩下10字节等内容,不是完整的消息解析不了
		//解决移出解析了的空间内容，继续写剩下的内容 	
